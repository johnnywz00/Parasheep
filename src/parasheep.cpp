...
void State::setUpHighScores () {
   ...
   ifstream scores { "parasheepscores.txt" };
   string line;
...
void State::loadSounds () {
    ...
    Sound snd;
    string basePath = "resources/";
    int sz = int(soundList.size());
    for (int i = 0; i <  sz ; ++i) {
        string fileName = basePath + soundList[i];
        loadByMethod(buf, fileName);
        buffers.push_back(buf);
        snd = Sound();
        snd.setBuffer(buffers[i]);
...
void State::loadFonts () {
    ...
    loadByMethod(levelFont, "resources/BlitzkriegNF.otf");
    loadByMethod(titleFont, "resources/BrassettBold.otf");
    loadByMethod(labelFont, "resources/BlitzkriegNF.otf");
}
...
void State::onCreate () {
    ...
    bkgdFrame.setTexture(texs[8]);
    bkgdFrame.sP(-550, -550);

    levelLabel = Text("LEVEL " + tS(level), levelFont, 30);
    ...
    platform.setOutlineColor(Color(131, 102, 3));
       ...
       // gradient sky
    for (int i = 0; i < 20; ++i) {
       bkgd[i].setSize(vecF(ScrW, ScrH / 20));
       bkgd[i].sP(0, (ScrH * (19 - i)) / 20);
    }
    for (int i = 0; i < 10; ++i) {
       clouds[i].setTexture(texs[6]);
    ...
    powerBar[1].setPosition(13, ScrCY + powerBar[1].gLB().height / 2);
       ...
       //== MENU SETUP == //
    menuBkgd.setSize(vecF(ScrW, ScrH));
...
void State::topTenToFile () {
   ...
   ofstream f { "parasheepscores.txt", ios_base::trunc };
   if (!f.is_open())
       return;
   for (auto& t : topTen) {
       f << t.score << " " << t.player << " " <<
...
const vector<string> State::texList {
    "launcher.png",     // 0
    "proj.png",         // 1
    "sheepL.png",       // 2
    "sheeppara.png",    // 3
...
const vector<string> State::soundList {
    "hit1.wav",         // 0
    "lose5.wav",        // 1
    "jump3.wav",        // 2
    "gameover2.wav",    // 3
